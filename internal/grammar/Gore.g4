...
context
    : 'condition:' conditionList ';' #conditionCheck
    | 'run:' statementList ';'       #runGore
    ;
...
statement
    : incDecStmt #incDec
    | key=VARIABLE ASSIGN value=expression #assign
    | 'return' #return
    | ifStmt #if
    | 'try:' statementList 'exception:' statementList #try
    | BINGO #bingo
    ;

ifStmt
    : 'if' cond=expression thenStmt ('else' (ifStmt | thenStmt))?
    ;

thenStmt
    : 'then' (statement eos)* 'end'
    ;

incDecStmt
    : VARIABLE (PLUS_PLUS | MINUS_MINUS)
    ;
...
unaryExp
    : unaryOper unaryExp #unaryOp
    | primaryExp         #primary
    ;

primaryExp
    : operand #oper
    | CONVERT_TYPE '(' expression (',' basicLit)? ')' #convert
    | 'round' '(' expression (',' prec=DECIMAL_LIT)? ')' #round
    ;
...
CONVERT_TYPE : 'int' | 'float' | 'bool' | 'string' | 'timestamp';
